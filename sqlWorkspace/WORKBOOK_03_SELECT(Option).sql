...
WHERE SUBSTR(G.TERM_NO, 1, 4) = '2007' AND 
        CLASS_NO = (
            SELECT CLASS_NO
            FROM TB_CLASS
...
SELECT S.STUDENT_NO, S.STUDENT_NAME, S.DEPARTMENT_NO, D.DEPARTMENT_NAME, ROUND(AVG(G.POINT), 1)
FROM TB_STUDENT S
JOIN TB_DEPARTMENT D ON (S.DEPARTMENT_NO = D.DEPARTMENT_NO)
JOIN TB_GRADE G ON (S.STUDENT_NO = G.STUDENT_NO)
WHERE S.ABSENCE_YN = 'N'
GROUP BY S.STUDENT_NO, S.STUDENT_NAME, S.DEPARTMENT_NO, D.DEPARTMENT_NAME
