...
SELECT PROFESSOR_NAME AS 교수이름
    , TRUNC ( ( MONTHS_BETWEEN( SYSDATE, 19||SUBSTR(PROFESSOR_SSN, 1, 6) ) ) / 12 ) AS 만나이
    , PROFESSOR_SSN
FROM TB_PROFESSOR
WHERE SUBSTR(PROFESSOR_SSN, 8, 1) = 1
ORDER BY 만나이 DESC, PROFESSOR_NAME;
...
FROM TB_STUDENT
WHERE MONTHS_BETWEEN( ENTRANCE_DATE, 19||SUBSTR(STUDENT_SSN, 1, 6) ) / 12 > 19 AND
    MONTHS_BETWEEN( ENTRANCE_DATE, 19||SUBSTR(STUDENT_SSN, 1, 6) ) / 12 <= 20;
