...
-- 7번
-- TB_CATAGORY 테이블과 TB_CLASS_TYPE 테이블의 PRIMARY_KEY 이름을 다음과 같이 변경하시오
-- PRIMARY KEY의 이름은 "PK_ + 컬럼이름"으로 지정하시오
ALTER TABLE TB_CATEGORY RENAME CONSTRAINT TB_CATEGORY_NAME_PK TO PK_CATEGORY_NAME;
ALTER TABLE TB_CLASS_TYPE RENAME CONSTRAINT SYS_C008348 TO PK_CLASS_TYPE_NO;
...
-- 10번
-- 춘 기술대학교 학생들의 정보만이 포함되어 있는 학생일반정보 VIEW를 만들고자 한다. (학번, 학생이름, 주소)
...
CREATE VIEW CCC
AS
    SELECT D.DEPARTMENT_NO, D.DEPARTMENT_NAME, COUNT(*) AS 학생수
